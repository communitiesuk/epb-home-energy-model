...
impl HeatPump {
    ...
    pub(crate) fn create_service_space_heating(
        ...
        control: SetpointOrCombinationControl,
        volume_heated: f64,
    ) -> anyhow::Result<HeatPumpServiceSpace> {
                ...
                .contains(&ResultParamValue::Empty)
            {
                ...
                *results_annual
                ...
                if results_annual["Overall"][&("energy_delivered_H5".into(), Some("kWh".into()))]
                    != ResultParamValue::Empty
                {
                    let service_energy_delivered_results = results_annual[&service_name]
                        [&("energy_delivered_H5".into(), Some("kWh".into()))]
                        .clone();
...
impl HeatPumpHotWaterOnly {
    ...
    pub fn demand_energy(
        ...
        let energy_supplied = if self.control_min.is_on(&simtime) {
    ...
    pub fn energy_output_max(&self, _temp_return: f64, simtime: SimulationTimeIteration) -> f64 {
        // Account for time control. In the Python they also check here whether control_min is None
        // but this is not possible as it's a required field for a HeatPumpHotWaterOnly object.
        if self.control_min.is_on(&simtime) {
...
mod tests {
    ...
    use crate::input::{
        BoilerHotWaterTest, BoilerType, CombiTypeSpecificDetails, FuelType, HeatPumpBufferTank,
        HeatSourceLocation,
    };
    ...
    fn heat_pump_service_space(
        ...
        let control = create_setpoint_time_control_setpoint_or_combination(vec![Some(20.), None]);
    ...
    // skipping Python's test_control_errors as covered by stricter types in Rust
    // skipping Python's test_energy_output_max due to mocking
    ...
    fn test_energy_output_max_off_for_service_space(
        ...
        let control = create_setpoint_time_control_setpoint_or_combination(vec![None, None]);
        heat_pump_service_space.control = control;
    ...
    fn heat_pump_warm_air(
        ...
        let control = create_setpoint_time_control_setpoint_or_combination(vec![None]);
    ...
    fn test_create_service_space_heating_warm_air(
        ...
        let control = create_setpoint_time_control_setpoint_or_combination(vec![]);
    ...
    fn test_backup_energy_output_max(
        ...
        let control_schedule = vec![Some(21.), Some(22.)];
        let control = SetpointOrCombinationControl::SetpointTime(
            SetpointTimeControl::new(
                control_schedule.clone(),
                0,
                1.,
                Default::default(),
                Default::default(),
    ...
    fn test_run_demand_energy_calc_other(
        ...
        let control =
            create_setpoint_time_control_setpoint_or_combination(vec![Some(21.), Some(22.)]);
    ...
    fn test_demand_energy_on_heat_pump_warm_air(
        ...
        let service_name = "service_space_warmair";
        let control = create_setpoint_time_control_setpoint_or_combination(vec![Some(20.)]);
        let volume_heated = 250.;
        let frac_convective = 0.9;
    ...
    fn test_output_detailed_results_water(
        ...
        let mut expected_results_per_timestep: ResultsPerTimestep = indexmap! {
            ...
            "servicetimestep_demand_energy".into() => indexmap! {
                ("service_name".into(), None) => vec![ResultParamValue::String(arcstr::literal!("servicetimestep_demand_energy")); 2],
                ("service_type".into(), None) => vec![ResultParamValue::String(arcstr::literal!("domestic_hot_water_regular")); 2],
    ...
    fn test_backup_only_operation(
        ...
        let control =
            create_setpoint_time_control_setpoint_or_combination(vec![Some(20.), Some(20.)]);
    ...
    fn test_setpnt_errors(simulation_time_for_heat_pump: SimulationTime) {
        ...
        hp1.control_min = Control::OnOffTime(OnOffTimeControl::new(vec![Some(true)], 0, 1.).into());

        assert!(hp1.setpnt(simtime).is_err());

        let mut hp2 = heat_pump.clone();
        hp2.control_max = Control::OnOffTime(OnOffTimeControl::new(vec![Some(true)], 0, 1.).into());
