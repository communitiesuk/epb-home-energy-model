...
#[derive(Clone, Copy, Debug, Default, Deserialize_enum_str, PartialEq, Serialize_enum_str)]
#[cfg_attr(feature = "arbitrary", derive(arbitrary::Arbitrary))]
...
pub enum HotWaterSourceDetails {
    ...
    CombiBoiler {
        ...
        //combi_storage_type: Annotated[
        //    CombiStorageType | None,
        //    Field(
        //        description="For storage combis, whether the store is primary or secondary",
        //    ),
        //] = None
...
fn validate_dry_core_output(
    ...
    // Validate that both SOC arrays start at 0.0 and end at 1.0
    if !relative_eq!(*soc_values.first().unwrap(), 0., max_relative = 1e-9) {
        return custom_validation_error(format!(
            "The first SOC value in {field} must be 0.0 (fully discharged)."
        ));
    }

    if !relative_eq!(*soc_values.last().unwrap(), 1., max_relative = 1e-9) {
        return custom_validation_error(format!(
            "The last SOC value in {field} must be 1.0 (fully charged)."
        ));
    }
...
fn validate_area_height_width(
    ...
    match data {
        ...
        } if !relative_eq!(*area, height_and_width.area(), max_relative = 1e-9) => custom_validation_error(
            format!(
